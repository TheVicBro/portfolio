...
const MobileMenu = memo(function MobileMenu({ isOpen, onClose, portfolioRef, careerRef, aboutRef, contactRef, router }: MobileMenuProps) {
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          {/* Close button */}
          <motion.button
            className="absolute top-8 right-8 text-white text-4xl z-10 p-2 w-14 h-14 flex items-center justify-center"
            onClick={onClose}
            whileTap={{ scale: 0.9 }}
          ...
          >
            {menuItems.map((item, index) => (
              ...
              >
                <motion.button
                  className="text-white text-5xl font-bold relative z-10 py-2 px-4"
                ...
                >
                  <span>{item}</span>
                  <motion.div 
                    className="absolute bottom-0 left-0 h-2 bg-skyblue"
                    variants={{
                      hover: { width: "100%" },
                      initial: { width: 0 }
                    }}
